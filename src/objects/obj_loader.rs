...
pub fn load_model(model_path: &str) -> RenderObject {
    ...
    let ranges = get_ranges(&obj_data.groups);
    let meshes = create_meshes(ranges, obj_model);  
    RenderObject::from_meshes(meshes)

    // let folder_path = Path::new(model_path).parent().unwrap().to_str().unwrap();
    // let mtl_path = format!("{}/{}", folder_path, &obj.material_libraries[0]);
    // let material = load_material(&mtl_path);
}

fn get_ranges(groups: &HashMap<String, raw::object::Group>) -> Vec<(usize, usize)> {
...
fn create_meshes(ranges: Vec<(usize, usize)>, 
